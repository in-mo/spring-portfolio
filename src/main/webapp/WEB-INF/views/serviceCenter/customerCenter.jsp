...
<script>
		...
		$.ajax({
			...
			success:function(res) {
				...
				if(res.qnaVo.status == '답변대기') {
					if('${ id }' == 'admin') {				
						str +=`
									<button type="button" class="btn btn-dark" onclick="qnaReplyAddBtn(event)">답글쓰기</button>
						`;
					}
				...
				if(res.qnaVo.status == '답변완료') {
					...
					if('${ id }' == 'admin') {	
						str +=`
									<button type="button" class="btn btn-dark" onclick="qnaReplyModifyBtn(event)">수정하기</button>
	...
	function qnaReplyDeleteBtn(event) {
		...
		let changeTag = $(event.currentTarget).parent().parent().parent().parent().prev().children('.status');
